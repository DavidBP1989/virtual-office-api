    ...
    public class Repository : IPermisos
    {
        ...
        public async Task<Dictionary<string, string>> GetCorteTiempoToDownload(int permissionId)
        {
            ...
            var corteTiempo = await _context.CorteTiempos
                .Join(_context.InformacionLaborals, c => c.EmpleadoId, i => i.EmpleadoId, (c, i) => new { c, i.IdCatalogoProyecto, i.StatusBorrado })
                ...
                .Where(x => x.cie.c.Id == permissionId && x.cie.StatusBorrado == 0)
                .Select(s => new CorteTiempoDownload
                {
                    ...
                    FechaSolicitud = s.cie.c.FechaSolicitud.GetValueOrDefault().ToString("dd MMMM yyyy", new CultureInfo("es-MX")),
        ...
        public async Task<Dictionary<string, string>> GetPermisoEconomicoDownload(int permissionId)
        {
            ...
            var permisoEconomico = await _context.PermisoEconomicos
                .Join(_context.InformacionLaborals, pe => pe.EmpleadoId, i => i.EmpleadoId, (pe, i) => new { pe, i.IdCatalogoProyecto, i.DenominacionPlaza, i.StatusBorrado })
                ...
                .Where(x => x.pee.pe.Id == permissionId && x.pee.StatusBorrado == 0)
                .Select(s => new PermisoEconomicoDownload
                {
                    ...
                    NombreEmpleado = $"{s.Nombres} {s.PrimerApellido} {s.SegundoApellido}",
                    FechaSolicitud = s.pee.pe.FechaRegistro.GetValueOrDefault().ToString("dd MMMM yyyy", new CultureInfo("es-MX")),
                    FechaInicio = s.pee.pe.FechaSolicitud.GetValueOrDefault().ToString("dd MMMM yyyy", new CultureInfo("es-MX")),
        ...
        public async Task<bool> PermiteAutorizarTodo() => await _context.AutorizacionSolicitudes.AnyAsync(x => x.AutorizaPermiso == _user.GetCurrentUser());
